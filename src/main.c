...
void remove_extension(char *filename) {
    ...
    if (dot && dot != filename) {       // Проверка наличия расширения
...
size_t rle_decompress(const unsigned char *input, size_t length, unsigned char **output) {
    ...
    *output = malloc(buffer_size);                // Выделение памяти для декодированных данных
    ...
    size_t out_pos = 0;                           // Текущая позиция в буфере
    for (size_t i = 0; i < length; i += 2) {      // Итерация по входным данным
...
void compress_directory(const char *path, FILE *output) {
    ...
    if (!dir) return;                              // Если не удалось открыть директорию, выходим

    while ((entry = readdir(dir)) != NULL) {       // Чтение всех записей в директории
        ...
        if (S_ISDIR(st.st_mode)) {                 // Если это директория, рекурсивно сжимаем ее
            compress_directory(full_path, output);
        } else {
            FileEntry *file_entry = read_file(full_path);  // Чтение файла в структуру FileEntry
            if (!file_entry) continue;                     // Пропуск, если не удалось прочитать файл
...
void decompress_directory(const char *output_path, FILE *input) {
    while (!feof(input)) {                           // Чтение до конца файла
        FileEntry file_entry;

        if (fread(file_entry.path, sizeof(file_entry.path), 1, input) != 1) break;
...
int is_compressed_directory(FILE *input) {
    ...
    if (fread(first_path, sizeof(first_path), 1, input) != 1) return 0;
...
void compress_multiple_files(int file_count, char *file_paths[], FILE *output) {
    for (int i = 0; i < file_count; i++) {
        ...
        if (!file_entry) continue;  // Пропуск, если файл не удалось прочитать
...
void decompress_in_current_directory(FILE *input) {
    while (!feof(input)) {  // Чтение до конца сжатого файла
        FileEntry file_entry;
...
int main(int argc, char *argv[]) {
    if (argc < 3) { // Проверка правильности количества аргументов
